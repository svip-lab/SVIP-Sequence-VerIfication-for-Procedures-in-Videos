...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, truncate=False):
...
def _resnet(arch, block, layers, pretrain, truncate, progress, **kwargs):
    ...
    if pretrain:
        if pretrain[-4:] == '.pth':
            ...
            model.load_state_dict({k[9:]:v for k,v in state_dict.items() if k[9:] in model.state_dict() and k[9:][:2] != 'fc'}, strict=False)

        else:
            logger.info('Wrong postfix of pretrain model path, please check!')
            exit(-1)
        logger.info('Loading backbone state_dict from %s' % pretrain)
    return model


def resnet18(pretrain=None, truncate=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrain, truncate, progress,
                   **kwargs)


def resnet34(pretrain=None, truncate=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet34', BasicBlock, [3, 4, 6, 3], pretrain, truncate, progress,
                   **kwargs)


def resnet50(pretrain=None, truncate=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrain, truncate, progress,
                   **kwargs)


def resnet101(pretrain=None, truncate=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet101', Bottleneck, [3, 4, 23, 3], pretrain, truncate, progress,
                   **kwargs)
...
def test():
    net = resnet50()
    print("resnet18 have {} paramerters in total".format(sum(x.numel() for x in net.parameters())))
